...
int WINAPI WinMain(_In_ HINSTANCE, _In_opt_ HINSTANCE, _In_ LPSTR, _In_ int) {
	...
	*/
	...
	while (Novice::ProcessMessage() == 0) {
		...
		Vector3 tempMin = aabb1.min;
		Vector3 tempMax = aabb1.max;

		aabb1.min.x = (std::min)(tempMin.x, tempMax.x);
		aabb1.max.x = (std::max)(tempMin.x, tempMax.x);
		aabb1.min.y = (std::min)(tempMin.y, tempMax.y);
		aabb1.max.y = (std::max)(tempMin.y, tempMax.y);
		aabb1.min.z = (std::min)(tempMin.z, tempMax.z);
		aabb1.max.z = (std::max)(tempMin.z, tempMax.z);

		
		Vector3 tempMin2 = aabb2.min;
		Vector3 tempMax2 = aabb2.max;
		aabb2.min.x = (std::min)(tempMin2.x, tempMax2.x);
		aabb2.max.x = (std::max)(tempMin2.x, tempMax2.x);
		aabb2.min.y = (std::min)(tempMin2.y, tempMax2.y);
		aabb2.max.y = (std::max)(tempMin2.y, tempMax2.y);
		aabb2.min.z = (std::min)(tempMin2.z, tempMax2.z);
		aabb2.max.z = (std::max)(tempMin2.z, tempMax2.z);
